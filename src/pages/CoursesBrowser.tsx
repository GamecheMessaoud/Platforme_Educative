...
import {
    ...
    Sparkles, Clock, Target, Trophy, Filter, ArrowLeft, Rocket
} from 'lucide-react';
...
export default function CoursesBrowser() {
    ...
    const bg = isDark ? 'bg-[#0d1117] text-slate-100' : 'bg-surface text-slate-900';
    ...
    return (
        <div className={`min-h-screen ${bg} overflow-hidden relative transition-colors duration-500 pb-32`} dir="rtl">
            {/* ══════ DECORATIVE BACKGROUND ══════ */}
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-[-10%] left-[-5%] w-[40rem] h-[40rem] bg-primary/10 rounded-full blur-[120px] animate-pulse-slow" />
                <div className="absolute bottom-[-10%] right-[-5%] w-[35rem] h-[35rem] bg-secondary/10 rounded-full blur-[100px] animate-pulse-slow" style={{ animationDelay: '2s' }} />

                {/* Floating Shapes */}
                <div className="absolute top-[20%] right-[10%] w-32 h-32 bg-gradient-to-br from-primary/20 to-transparent rounded-[2.5rem] rotate-12 animate-float shadow-xl backdrop-blur-sm border border-white/10" />
                <div className="absolute bottom-[15%] left-[5%] w-24 h-24 bg-gradient-to-tr from-accent/20 to-transparent rounded-full animate-float-3d shadow-xl backdrop-blur-sm border border-white/10" style={{ animationDelay: '1.5s' }} />
                <div className="absolute top-[60%] right-[5%] w-20 h-20 bg-gradient-to-bl from-emerald-500/10 to-transparent rounded-3xl -rotate-12 animate-orbit shadow-lg" />
            </div>

            {/* ══════ HEADER ══════ */}
            <header className={`sticky top-0 z-40 backdrop-blur-2xl border-b ${isDark ? 'bg-slate-900/60 border-slate-800' : 'bg-white/60 border-slate-100'}`}>
                <div className="max-w-[1500px] mx-auto px-8 py-8 flex items-center justify-between">
                    <div className="flex items-center gap-8">
                        <button onClick={() => navigate(isAuthenticated ? '/student-dashboard' : '/')}
                        ...
                        <div>
                            <h1 className="text-4xl font-black tracking-tighter text-gradient pb-1">استكشف المسارات الإبداعية</h1>
                            <p className={`text-base font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>أكثر من 20 مسار تعليمي متميز صمم خصيصاً لك ✨</p>
                        </div>
                    </div>
            ...
            <main className="max-w-[1500px] mx-auto px-8 py-16 space-y-16 relative z-10">
                {/* ══════ SEARCH ══════ */}
                            ...
                            >
                                ...
                                <div className="h-80 relative overflow-hidden bg-slate-100">
                                    ...
                                    <div className="absolute top-8 right-8 flex flex-col gap-3">
                                        <div className="bg-primary/90 backdrop-blur-xl border-2 border-white/20 text-white px-6 py-3 rounded-2xl text-[11px] font-black shadow-2xl uppercase tracking-widest animate-premium-in">
                                ...
                                <div className="p-10 flex-1 flex flex-col space-y-8 relative">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {course.isRecommended && (
                                                    <span className="text-[10px] uppercase tracking-widest font-black px-4 py-2 rounded-2xl bg-indigo-500 text-white shadow-lg animate-pulse-slow">
                                                        ✨ مقترح لمستواك
                                                    </span>
                                                )}
                                                {course.isLocked && (
                                                    <span className="text-[10px] uppercase tracking-widest font-black px-4 py-2 rounded-2xl bg-slate-800 text-slate-300 shadow-md">
                                                        🔒 يفوق مستواك
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    <div className="pt-6 mt-auto">
                                        {course.isEnrolled ? (
                                            ...
                                            <button
                                                disabled
                                                className={`w-full h-20 flex items-center justify-center gap-4 rounded-[2.5rem] font-black text-xl transition-all duration-700 opacity-60 cursor-not-allowed ${isDark ? 'bg-slate-800 text-slate-400 border-2 border-slate-700' : 'bg-slate-200 text-slate-500'}`}
                                            >
                                                <span>🔒 الدورة تفوق مستواك الحالي</span>
                                            </button>
                                        ) : (
                                            <button
                                            ...
                                            >
                                                <div className="absolute inset-0 bg-white/20 translate-y-20 group-hover/btn:translate-y-0 transition-transform duration-500" />
                                                <Search size={32} className="relative z-10" /> <span className="relative z-10">عرض التفاصيل</span>
                                            </button>
                                        )}
                                    </div>
                                </div>
